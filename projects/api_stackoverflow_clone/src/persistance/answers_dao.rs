...
impl AnswersDao for AnswersDaoImpl {
    async fn create_answer(&self, answer: Answer) -> Result<AnswerDetail, DBError> {
        let uuid = sqlx::types::Uuid::parse_str(answer.question_uuid.as_str())
    ...
    async fn delete_answer(&self, answer_uuid: String) -> Result<(), DBError> {
        let uuid = sqlx::types::Uuid::parse_str(answer_uuid.as_str())
        ...
        sqlx::query!("DELETE FROM answers WHERE answer_uuid = $1", uuid)
            .execute(&self.db)
            .await
            .map_err(|e: sqlx::error::Error| { DBError::Other(Box::new(e)) })?;

        Ok(())
    }

    async fn get_answers(&self, question_uuid: String) -> Result<Vec<AnswerDetail>, DBError> {
        let uuid = sqlx::types::Uuid::parse_str(question_uuid.as_str())
            .map_err(|_| DBError::InvalidUUID(format!("Could not parse answer UUID: {}", question_uuid)))?;
