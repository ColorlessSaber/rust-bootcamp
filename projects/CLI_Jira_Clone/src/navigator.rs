    ...
    pub fn handle_action(&mut self, action: Action) -> Result<()> {
        match action {
            ...
            Action::CreateEpic => {
                ...
                self.db.create_epic(new_epic).with_context(|| anyhow!("Failed to create new Epic"))?;
            }
            Action::UpdateEpicStatus { epic_id } => {
                let new_epic_status = (self.prompts.update_status)();
                if let Some(new_epic_status) = new_epic_status {
                    self.db.update_epic_status(epic_id, new_epic_status).with_context(|| anyhow!("Failed to update epic status"))?;
                }
            }
            ...
            Action::CreateStory { epic_id } => {
                ...
                self.db.create_story(new_story, epic_id).with_context(|| anyhow!("Failed to add new Story to db"))?;
            }
            Action::UpdateStoryStatus { story_id } => {
                let new_story_status = (self.prompts.update_status)();
                if let Some(new_story_status) = new_story_status {
                    self.db.update_story_status(story_id, new_story_status).with_context(|| anyhow!("Failed to update story status"))?;
                }
            }
